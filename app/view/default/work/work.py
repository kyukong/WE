...
def work_search():
    ...
    if work_list['result'] != 'fail':
        work_total_count = work_list['count']
    ...
    return render_template('/work/template_workSearch.html', menu_list=menu_list, now_top_menu=now_top_menu, 
...
def work_detail(day):
    ...
    plan_list = Work().get_plan_info(day, user_id)
    ...
    work_list = Work().get_work_info(day, user_id)
    if work_list['result'] != 'fail':
        work_list = work_list['data']
    else:
        work_list = []
