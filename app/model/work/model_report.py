...
class Report():
    ...
    # 20211011 KYB add 보고서 정보 조회
    def get_report_list(self, search_list):
        '''
        ...
        try:
            ...
            sql += f"SELECT REPORT_ID, IFNULL(`report`.REGISTER_DATETIME, '') AS REGISTER_DATETIME, " \
                   ...
                   f"IFNULL((SELECT CODE_NAME FROM tc_code_info WHERE `CODE` = PAYMENT_PROGRESS_CODE), '') AS PAYMENT_PROGRESS_CODE_NAME, " \
    ...
    # 20211017 KYB add 보고서 정보 조회
    def get_report_info(self, plan_date, user_id):
        '''
        보고서 상세정보 조회 화면에서 보고서 정보 조회
